...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        U, sigma_mat, V = svd(X, full_matrices=False)
        sigma_mat = np.diag(sigma_mat)
        for i in range(sigma_mat.shape[0]):
            sigma_mat[i][i] = sigma_mat[i][i] /(sigma_mat[i][i]**2+self.lam_)
        self.coefs_ = V.T @ sigma_mat @ U.T @ y
